...
io.attach(app, {
  //include local domain to avoid CORS error locally
  //configure accordingly for production
  cors: {
    origin: "http://localhost",
    methods: ["GET", "POST"],
    credentials: true,
    transporsts: ["websocket", "polling"],
  },
  allowEIO3: true,
});

io.on("connection", (socket) => {
  console.log("Now socket is connected >>", socket.id);
});
